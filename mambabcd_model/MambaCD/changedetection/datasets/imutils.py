...
#Standarization using my data
# def normalize_img(img, mean=[93.33933, 141.12399, 116.26321, 708.13832], 
#                  std=[183.36842, 266.83552, 253.25570, 1253.23890]):

#modify to use standardization from band 8
def normalize_img(img, 
                  mean=[432.091472, 619.178328, 602.210604, 2540.547798],
                  std=[236.200339, 299.175738, 442.481782, 809.148702]):
    """Normalize image by subtracting the mean and dividing by the standard deviation for each band.
    
    The bands correspond to:
    - Band 2: mean=432.091472, std=236.200339
    - Band 3: mean=619.178328, std=299.175738
    - Band 4: mean=602.210604, std=442.481782
    - Band 8: mean=2540.547798, std=809.148702
    """
    img_array = np.asarray(img)
    normalized_img = np.empty_like(img_array, dtype=np.float32)
    
    # Loop over channels corresponding to the 4 bands
    for i in range(4):
...
def fliplr(pre_img, post_img, label):
...
def flipud(pre_img, post_img, label):
