...
class EinFFT(nn.Module):
    def __init__(self, dim, drop_rate=.7):
        super().__init__()
        self.hidden_size = dim #768
        self.num_blocks = 4 
        self.block_size = self.hidden_size // self.num_blocks 
        assert self.hidden_size % self.num_blocks == 0
        self.sparsity_threshold = 0.09508911964533545
        self.scale = 0.0011906415270652637
...
class SS2D(nn.Module):
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if x.device != self.in_proj.weight.device:
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.mlp_branch:
            ...
            if cm_type == "EinFFT":  # Use EinFFT if cm_type is "EinFFT"
                self.mlp = EinFFT(hidden_dim)
                print(f"Using EinFFT in VSSBlock (cm_type={cm_type})")
            else:  # Otherwise, use MLP
                _MLP = Mlp if not gmlp else gMlp
                mlp_hidden_dim = int(hidden_dim * mlp_ratio)
                self.mlp = _MLP(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=mlp_act_layer, drop=mlp_drop_rate, channels_first=channel_first)
                print(f"Using MLP in VSSBlock (cm_type={cm_type})") 
    ...
    #     return x
    def _forward(self, input: torch.Tensor):
        # Store the input device for consistency
        input_device = input.device
        
        # --- SSM branch ---
        if self.ssm_branch:
            if self.post_norm:
                # Ensure op operates on input's device
                op_out = self.op(input.to(self.op.in_proj.weight.device))
                # Move result back to input device if needed
                if op_out.device != input_device:
                    op_out = op_out.to(input_device)
                ssm_out = self.norm(op_out)
                x = input + self.drop_path(ssm_out)
            else:
                norm_out = self.norm(input)
                # Ensure op operates on norm_out's device
                op_out = self.op(norm_out.to(self.op.in_proj.weight.device))
                # Move result back to input device if needed
                if op_out.device != input_device:
                    op_out = op_out.to(input_device)
                x = input + self.drop_path(op_out)
        else:
            x = input
        ...
        if self.mlp_branch:
            if isinstance(self.mlp, EinFFT):
                ...
                # Ensure x_reshaped is on input_device
                if x_reshaped.device != input_device:
                    x_reshaped = x_reshaped.to(input_device)
                    
                # Assume square spatial dims: N = H*W.
                H = W = int(math.sqrt(x_reshaped.shape[1]))
                normed2 = self.norm2(x_reshaped)
                
                # Ensure mlp operates on normed2's device
                mlp_out = self.mlp(normed2, H, W)
                
                # Ensure mlp_out is on input_device
                if mlp_out.device != input_device:
                    mlp_out = mlp_out.to(input_device)
                    
                # Reshape mixer output back to (B, H, W, C)
                mlp_out = mlp_out.view(B, H, W, C)
                
                if self.post_norm:
                    norm2_out = self.norm2(mlp_out)
                    if norm2_out.device != input_device:
                        norm2_out = norm2_out.to(input_device)
                    x = x + self.drop_path(norm2_out)
                else:
                    if mlp_out.device != input_device:
                        mlp_out = mlp_out.to(input_device)
                    x = x + self.drop_path(mlp_out)
        ...
        # Final check to ensure output is on input device
        ...
        **kwargs,
        ...
        for d in range(depth):
            blocks.append(VSSBlock(
                ...
                gmlp=gmlp,
                use_checkpoint=use_checkpoint,
                #Where to specify encoder channel mixer
                cm_type="mlp", #EinFFT, mlp
            ))
