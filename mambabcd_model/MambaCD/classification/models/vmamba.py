...
class VSSBlock(nn.Module):
    def __init__(
        ...
        cm_type: str = "EinFFT",  # options: "mlp" or "EinFFT"
        # =============================
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.mlp_branch:
            if isinstance(self.mlp, EinFFT):
                #print("Before channel mixing, x shape:", x.shape)
                # Ensure x is in (B, N, C) with N = H*W.
                if len(x.shape) == 4:
                    if self.channel_first:
                        B, C, H, W = x.shape
                        #print("Detected channel_first (B, C, H, W):", x.shape)
                        x_reshaped = x.permute(0, 2, 3, 1).contiguous().view(B, H * W, C)
                        #print("After permute and reshape (B, H*W, C):", x_reshaped.shape)
                    else:
                        B, H, W, C = x.shape
                        #print("Detected non-channel_first (B, H, W, C):", x.shape)
                        x_reshaped = x.view(B, H * W, C)
                        #print("After reshape (B, H*W, C):", x_reshaped.shape)
                elif len(x.shape) == 3:
                    B, N, C = x.shape
                    x_reshaped = x
                    H = W = int(math.sqrt(N))
                    #print("Input already 3D, inferred H and W:", H, W)
                else:
                ...
                #print("After EinFFT, mlp_out shape:", mlp_out.shape)
                # Reshape mixer output back to (B, H, W, C)
                mlp_out = mlp_out.view(B, H, W, C)
                #print("After reshaping back (B, H, W, C):", mlp_out.shape)
                if self.post_norm:
