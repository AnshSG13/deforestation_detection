...
class EinFFT(nn.Module):
    def __init__(self, dim):
        ...
        # nn.init.kaiming_uniform_(self.complex_weight_1, mode='fan_in', nonlinearity='relu')
        # nn.init.kaiming_uniform_(self.complex_weight_2, mode='fan_in', nonlinearity='relu')
        # nn.init.kaiming_uniform_(self.complex_bias_1, mode='fan_in', nonlinearity='relu')
        # nn.init.kaiming_uniform_(self.complex_bias_2, mode='fan_in', nonlinearity='relu')
    ...
    def forward(self, x, H, W):
        ...
        if torch.cuda.is_available():
            print(f"[START] GPU memory: {torch.cuda.memory_allocated()/1e9:.2f} GB")
        
        B, N, C = x.shape
        #print(f"EinFFT input: shape={x.shape}, dtype={x.dtype}")
        ...
        # After FFT
        if torch.cuda.is_available():
            print(f"[After FFT] GPU memory: {torch.cuda.memory_allocated()/1e9:.2f} GB")
        
        #print(f"After FFT: shape={x.shape}, is_complex={torch.is_complex(x)}")
        ...
        if torch.cuda.is_available():
            print(f"[After complex ops] GPU memory: {torch.cuda.memory_allocated()/1e9:.2f} GB")
        ...
        # Before the critical conversion
        if torch.cuda.is_available():
            print(f"[Before float32 conversion] GPU memory: {torch.cuda.memory_allocated()/1e9:.2f} GB")
        
        #print("Before conversion to float32 - CRITICAL POINT")
        x = x.to(torch.float32)  # Line 62 in your error message
        
        # After conversion
        if torch.cuda.is_available():
            print(f"[After float32 conversion] GPU memory: {torch.cuda.memory_allocated()/1e9:.2f} GB")
        
        x = x.reshape(B, N, C)
        
        # At the end
        if torch.cuda.is_available():
            print(f"[END] GPU memory: {torch.cuda.memory_allocated()/1e9:.2f} GB")
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,channels_first=False):
        ...
        # Apply He Initialization
        nn.init.kaiming_uniform_(self.fc1.weight, mode='fan_in', nonlinearity='gelu')
        nn.init.kaiming_uniform_(self.fc2.weight, mode='fan_in', nonlinearity='gelu')
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if forward_type.startswith("v0"):
            ...
            return
        self.apply(self._init_weights_kaiming)
        
    def _init_weights_kaiming(self, m: nn.Module):
        # We only want to init typical conv/linear layers, not your specialized SSM parameters
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if not self.disable_z:
            x, z = x.chunk(2, dim=(1 if self.channel_first else -1))  # (B, h, w, d) or (B, d, h, w)
            if not self.disable_z_act:
                z = self.act(z)
        
        # Only permute if x is 4D.
        if not self.channel_first and x.dim() == 4:
            x = x.permute(0, 3, 1, 2).contiguous()
        if with_dconv:
            x = self.conv2d(x)  # (B, d, h, w)
        x = self.act(x)
...
#modified VSSBlock to have ensteinFFT
class VSSBlock(nn.Module):
    def __init__(
        ...
        gmlp=False,
        # New parameter: choose channel mixer type
        cm_type: str = "mlp",  # options: "mlp" or "EinFFT"
        # =============================
        ...
        **kwargs,
        ...
        if self.mlp_branch:
            ...
            if cm_type == "EinFFT":
                # Use EinFFT as the channel mixer
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.ssm_branch:
            if self.post_norm:
                ssm_out = self.norm(self.op(input))
                x = input + self.drop_path(ssm_out)
            else:
        ...
        if self.mlp_branch:
            if isinstance(self.mlp, EinFFT):
                # Ensure x is in (B, N, C) with N = H*W.
                if len(x.shape) == 4:
                    if self.channel_first:
                        B, C, H, W = x.shape
                ...
                # Assume square spatial dims: N = H*W.
                H = W = int(math.sqrt(x_reshaped.shape[1]))
                normed2 = self.norm2(x_reshaped)
                mlp_out = self.mlp(normed2, H, W)
                # Reshape mixer output back to (B, H, W, C)
                mlp_out = mlp_out.view(B, H, W, C)
                if self.post_norm:
                    x = x + self.drop_path(self.norm2(mlp_out))
                else:
