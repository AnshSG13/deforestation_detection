...
class CDEvaluator():
    ...
    def _update_metric(self):
        ...
        # If target has shape [N, 1, H, W], squeeze to [N, H, W]
        if target.dim() == 4:
            target = target.squeeze(1)
        
        G_pred = self.G_pred.detach()
        # Get predicted class per pixel, resulting in shape [N, H, W]
        G_pred = torch.argmax(G_pred, dim=1)

        # Convert to NumPy arrays
        gt_np = target.cpu().numpy()   # Now shape is [N, H, W]
        pred_np = G_pred.cpu().numpy()   # Shape [N, H, W]

        # Create a valid mask: True where ground truth is not 2
        valid_mask = (gt_np != 2)
        
        # Filter out the invalid pixels
        valid_gt = gt_np[valid_mask]
        valid_pred = pred_np[valid_mask]

        # Update the metric only with valid pixels
        current_score = self.running_metric.update_cm(pr=valid_pred, gt=valid_gt)
